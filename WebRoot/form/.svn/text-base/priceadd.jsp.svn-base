  ...
  <head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
             $('#priceAddForm input').keyup(trimkeyup);
 		});
	</script>
  </head>
				...
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td>
							<!-- 可以写form -->
							<form action="${pageContext.request.contextPath}/hod2000Price!doSave.do" method="post" id="priceAddForm">
								<input type="hidden" name="method" value="1"/>
								<table cellpadding="0" cellspacing="0" border="0" height="300px" align="center" id="tab">
									<tr>
										<td>方案名称：</td>
										<td><input id="priceName" name="hod2000Price.pname"/><span>必填，2-50位字符</span></td>
									</tr>
									<tr>
										<td>启用日期：</td>
										<td><input class="Wdate" type="text" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onfocus="WdatePicker({minDate:'%y-%M-{%d+1}'})" id="startTime" name="hod2000Price.pstartTime"/><span>必填</span></td>
									</tr>
									<tr>
										<td>基础费单价(元/月/平方米)：</td>
										<td><input type="text" id="pdBasePrice" name="hod2000PriceDetail.pdBasePrice" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/><span>必填，只能输入数值型字符</span></td>
									</tr>
									<tr>
										<td>用热单价(元/KWh)：</td>
										<td><input type="text" id="pdPowerPrice" name="hod2000PriceDetail.pdPowerPrice" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/><span>必填，只能输入数值型字符</span></td>
									</tr>
									<tr>
										<td>状态：</td>
										<td>
											<select name="hod2000Price.pstatus">
