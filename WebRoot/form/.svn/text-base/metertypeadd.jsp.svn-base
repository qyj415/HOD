  ...
  <head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
             $('#meterTypeForm input').keyup(trimkeyup);
 		});
		function checkForm()
		{
				...
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td>
							<!-- 可以写form -->
							<form action="${pageContext.request.contextPath}/hod2000MeterType!doSave.do" method="post" id="meterTypeForm">
								<input name="hod2000MeterType.companyNum" id="product_number" type="hidden"/>
								<table cellpadding="0" cellspacing="0" border="0" height="250px" align="center" id="tab">
									<tr>
										<td>表计型号：</td>
										<td><input type="text" id="meterType" name="hod2000MeterType.typeName" maxlength="20"/><span>必填，2-10位字符</span></td>
									</tr>
									<tr>
										<td>仪表类型：</td>
										<td><input id="meterCode" name="hod2000MeterType.typeCode" maxlength="2" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"><span>必填，2个数字</span></td>
									</tr>
									<tr>
										<td>厂商名称：</td>
										<td><input name="hod2000MeterType.companyName" id="product_name" maxlength="10"/><span>必填，2-10位字符</span></td>
									</tr>
									<tr>
										<td>厂商编号：</td>
										<td><input id="product_num1" maxlength="2" style="width: 49px;" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/>-<input id="product_num2" maxlength="2" style="width: 49px;" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/>-<input id="product_num3" maxlength="2" style="width: 49px;" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"/><span>必填，每个框2个数字，比如HOD为72-79-68</span></td>
									</tr>
									<tr>
										<td>通信规约：</td>
										<td>
											<select name="hod2000MeterType.communicationProtocol">
