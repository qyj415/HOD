...
<html>
  <head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
             $('#roleEditForm input').keyup(trimkeyup);
 		});
  ...
  <body>
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td>
							<!-- 可以写form -->
							<form action="${pageContext.request.contextPath}/hod2000Roles!doUpdate.do" method="post" name="frmoperatorType" id="roleEditForm">
								<input type="hidden" name="hod2000Role.rid" value="${hod2000Role.rid }"/>
								<table cellpadding="0" cellspacing="0" border="0" align="center" id="tab">
									<tr height="40">
										<td>角色名称：</td>
										<td colspan="3"><input id="roleName" value="${hod2000Role.rname }" name="rname"/><span>必填，2-16位字符</span></td>
									</tr>
									<tr height="270">
										<td>角色权限：</td>
										<td>
											<select multiple="multiple" style="width: 180px;height: 250px;margin-right: 20px;" id="rolePermissionLeft" ondblclick="dbMoveLeft()">
												<c:forEach items="${permissionListNotIn}" var="v">
										...
										<td>
											<select multiple="multiple" style="width: 180px;height: 250px;margin-left: 20px;" id="rolePermissionRight" name="rolePermissionRight" ondblclick="dbMoveRight()">
												<c:forEach items="${permissionList}" var="v">
													<option value="${v.permId }">${v.permName }</option>
												</c:forEach>
											</select><span>左边为待选权限，右边为已选权限</span>
										</td>
									</tr>
									<tr>
										<td align="center" colspan="4">
											<input type="submit" value="提交" onclick="return checkForm()"/>
											<input type="button" value="重置" onclick="javaScript:window.location.href='${pageContext.request.contextPath}/hod2000Roles!toUpdate.do?id=${hod2000Role.rid }';"/>
										</td>
									</tr>
								</table>
							</form>
  		...
  		<script type="text/javascript">
			var opetPurview ='${hod2000Role.rpurview}';
			var opetPurviews = opetPurview.split(";");
			for (var i=0;i<opetPurviews.length-1;i++)
			{
			     var cname = (opetPurviews[i].split(":")[0]);
			     for (var ii=0;ii<opetPurviews[i].split(":").length;ii++)
			     {
			  		for (var jj=0;jj<document.getElementsByName(cname).length;jj++)
	           		{
	           			 var e=document.getElementsByName(cname)[jj];
	            		 if (e.value ==  opetPurviews[i].split(":")[ii])
	             		{
	                  		e.checked=true;
	              		}
	           		}
