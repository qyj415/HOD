  ...
  <head>
	...
	<script type="text/javascript">
		...
		function addRow(row)
		{
            ...
            //newTR.cells[1].firstChild.value=0;
            //更新行号
            updNo();
		}
		//删除行
		...
		function checkForm()
		{
			...
			var priceValue="[{pid:${hod2000Price.pid},pname:'"+priceName+"',pstartTime:'"+pstartTime+"',pstatus:${hod2000Price.pstatus},ptype:${hod2000Price.ptype}}]";
			var tab=document.getElementById('list');
			var objs='[';
			for(var i=1;i<tab.rows.length;i++)
			{
				var pdId=$("#row"+i).find("#pdId").val();
				...
				objs+='{pdId:'+pdId+',pdBasePrice:'+basePrice+',pdPowerPrice:'+powerPrice+',pdPower:'+powerValue+'},';
			}
			objs=objs.substring(0,objs.length-1);
			objs+=']';
			location.href='${pageContext.request.contextPath}/hod2000Price!doUpdate.do?priceValue='+encodeURI(encodeURI(priceValue))+'&details='+objs+'&method=2';
		}
		$(function(){
			updNo();  
			$('#priceEdit1Form input').keyup(trimkeyup);
		});
	</script>
  </head>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
												...
												<tr>
													...
													<td width="90">
														<table width="99%" border="0" cellpadding="0"
															...
															<tr>
																<td class="STYLE2">
																	<a href="${pageContext.request.contextPath}/hod2000Price!doSelect.do?method=${hod2000Price.ptype }"><div class="returnbtn"></div></a>
																</td>
															</tr>
														</table>
													</td>
			...
			<tr>
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" id="priceEdit1Form">
						<tr>
							...
							<table cellpadding="0" cellspacing="0" border="0" align="center" id="tab">
								<tr height="40">
									<td>方案名称：</td>
									<td><input id="priceName" name="priceName" value="${hod2000Price.pname}"/><span>必填，2-50位字符</span></td>
								</tr>
								<tr height="40">
									<td>启用日期：</td>
									<td><f:formatDate value="${hod2000Price.pstartTime}" pattern="yyyy-MM-dd" var="startTime"/><input class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" onfocus="WdatePicker({minDate:'%y-%M-{%d+1}'})" readonly="readonly" id="startTime" name="hod2000Price.pstartTime" value="${startTime }"/><span>必填</span></td>
								</tr>
								<tr height="40">
									<td>基础费单价(元/月/平方米)：</td>
									<td>
										<input type="text" id="basePrice" value="${basePrice }" name="basePrice" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/><span>必填，只能输入数值型字符</span>
									</td>
								</tr>
								<tr height="210">
									<td colspan="2">
											...
											<table cellpadding="0" cellspacing="1" border="0" width="100%" bgcolor="b5d6e6" class="list" id="list">
												...
												<c:forEach items="${dataList}" var="v" varStatus="num">
													<tr>
														...
														<td><input type="hidden" value="${v.pdId }" id="pdId"/></td>
														<td class="STYLE1"><input type="text" name="powerValue" value="${v.pdPower }" id="powerValue" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/></td>
														<td class="STYLE1"><input type="text" name="powerPrice" value="${v.pdPowerPrice }" id="powerPrice" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/></td>
													</tr>
												</c:forEach>
								...
								<tr height="100">
									<td colspan="2">
										...
										<c:if test="${hod2000Price.ptype==2}">3.阶梯方案一：阶梯能量费=耗热量×阶梯单价。</c:if>
										<c:if test="${hod2000Price.ptype==3}">3.阶梯方案二：阶梯能量费=Σ（每一阶耗热量*该阶段阶梯单价）。</c:if>
									</td>
								</tr>	
							</table>
							</td>
