  ...
  <head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
             $('#roomEditForm input').keyup(trimkeyup);
             $('#roomEditForm textarea').keyup(trimkeyup);
 		});
	</script>
  </head>
				...
				<td>
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td>
							<!-- 可以写form -->
							<form action="${pageContext.request.contextPath}/hod2000Room!doUpdate.do" method="post" id="roomEditForm">
								<input type="hidden" value="${hod2000Room.roomId }" name="hod2000Room.roomId">
								<input type="hidden" value="${hod2000Room.hod2000Unit.unitId }" name="unitId">
								<table cellpadding="0" cellspacing="0" border="0" height="350px" width="50%" align="center" id="tab">
									<tr>
										<td>房间名称：</td>
										<td><input type="text" id="roomName" name="roomName" value="${hod2000Room.roomName }"/><span>必填，2-32位字符</span></td>
									</tr>
									<tr>
										<td>面积数(㎡)：</td>
										<td><input type="text" id="roomArea" name="roomSize" value="${hod2000Room.roomSize }" onkeyup="this.value=this.value.replace(/[^0-9\.]/g,'')"/><span>必填，1-20位字符</span></td>
									</tr>
									<tr>
										<td>户型：</td>
										<td>
											<input type="text" id="roomHouseType" name="roomHouseType" value="${hod2000Room.roomHouseType }"/><span>必填，2-32个字符</span>
										</td>
									</tr>
									<tr>
										<td>备注信息：</td>
										<td><textarea cols="30" rows="6" id="roomRemark" name="roomRemark">${hod2000Room.roomRemark }</textarea><span>最长不超过128个字符</span></td>
									</tr>
									<tr>
