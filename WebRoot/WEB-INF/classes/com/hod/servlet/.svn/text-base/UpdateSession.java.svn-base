...
public class UpdateSession extends HttpServlet {
	...
	static Logger log = Logger.getLogger(UpdateSession.class.getName());
	/**
...
//		Ipnet2klcOperator operator=(Ipnet2klcOperator)request.getSession().getAttribute("user");
//		log.info("当前用户："+operator.getOperLogginName());
		...
		UserCounter.updateUserCounter(request.getSession().getId(), 
				dateFormatYMdHms.format(new Date()), request.getRemoteHost(),""
				);
		out.flush();
		out.close();
	}
