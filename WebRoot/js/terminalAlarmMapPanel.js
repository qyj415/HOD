...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
				...
				/*
					...
					success : function(res) {
						...
						if (true == result.success) {
							var Pane = Ext.getCmp('terminalAlarmmap');
    ...
    showEventMarkBycomid:function(comid,eventid){  //显示有事件的标注
    	var index = null;
    	for(var i=0;i<this.markInfors.length;i++){
    		if(this.markInfors[i].comid===comid){
    				index = i;
					break;
    		}
    	}
    	...
    	if (this.markInfors[index].type==0){//楼栋
    		drawmark = new BMap.Marker(this.markInfors[index].point,{icon:this.terminalEvent_icon});		
    	}
    	this.markInfors[index].mark = drawmark;
    	this.markInfors[index].eventid = eventid;
    	this.AddMyListener(drawmark);	
    	this.getMap().addOverlay(drawmark);	
    },
    showEvnetByMark:function(mark){
    	...
    	if (index==null) return;
    	if (!this.eventinforDlg){
    		delete this.eventinforDlg;
    		this.eventinforDlg = null;
    	}
    	//删除有事件的标注,生成无事件的标注
    	mark.remove();
    	var drawmark=null;
    	if (this.markInfors[index].type==0){//楼栋
    		this.eventinforDlg = new gMapTerminalEventCreate;
    		drawmark = new BMap.Marker(this.markInfors[index].point,{icon:this.terminalEvent_icon});		
    	}
    	this.eventinforDlg.gridStore.load({
					params : {
						start : 0,
						limit :15,
		...
		this.eventinforDlg.show();		
		this.markInfors[index].mark = drawmark;
    	//this.markInfors[index].eventid = 0;
    	this.AddMyListener(drawmark);	
    	this.getMap().addOverlay(drawmark);
    },
    showevnet_menu : function(x,y) { // 显示事件右键菜单
    	if (this.event_menu==null){
			var showEvnetMenu = new Ext.menu.Item({
						iconCls : 'moveMark',
						text : '显示报警信息',
						handler : function(){
							this.event_menu.hide();
							this.showEvnetByMark(this.focusMark);
						}.createDelegate(this)
					});
			var menuList = [showEvnetMenu];
			this.event_menu = new Ext.menu.Menu({
						items : menuList
					});
    	};
    	this.event_menu.showAt([x,y]);
    },
    ...
    MoveStart:function(map){// 移动事件
    	map.addEventListener("movestart",function() {
			if (Ext.getCmp('terminalAlarmmap').DrawOperation==-1)	
    			this.initmap();
		}.createDelegate(this));
    },
    ...
    SingleRightClick:function(map){ //右键单击事件
    	map.addEventListener("rightclick",function(event){
    		...
    		else if(this.focusType>29)
    			this.showevnet_menu(x,y);//显示事件的右键菜单
    	}.createDelegate(this));	
    },
							...
							for (var i = 0; i < result.length; i++) {
								try {
									markobj = {
										...
										eventid : result[i].terminalEvent,
										mark : null
									};
			...
			for (var i = 0; i < this.markInfors.length; i++) {
				if (this.markInfors[i].type == 0) { // 楼栋
					if (this.ShowTerminal == true) {
						...
						{
							if (this.markInfors[i].eventid>0)
							DrawMark = new BMap.Marker(this.markInfors[i].point, {//有事件
										title :this.markInfors[i].name,
										icon : this.terminalEvent_icon
									});
							else
