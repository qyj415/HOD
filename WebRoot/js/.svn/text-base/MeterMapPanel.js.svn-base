...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
    ...
    showMControlInfor:function(mark){ //显示
    	if(!this.mcontrolDlg)
    	{
    		this.mcontrolDlg = new gMapMcontrolCreate;
    	}
    	...
    	if (null != index)
    	{
    		   //this.markInfors[index].comid
    		   this.mcontrolDlg.setTitle('热量表信息[' + this.markInfors[index].name + ']') ;
    		   this.mcontrolDlg.gridStore.load({
					params : {
						start : 0,
						limit :15,
						terminal: this.markInfors[index].comid
					}
				});
    	}

    	this.mcontrolDlg.show();
    },
    ...
    showterminal_menu : function(x,y) { // 楼栋右键菜单
    	...
    	if (this.terminal_menu==null){
			var showMControlMenu = new Ext.menu.Item({
						iconCls : 'showMControlInfor',
						text : '热量表信息',
						handler : function(){
						this.terminal_menu.hide();
						this.showMControlInfor(this.focusMark);
						}.createDelegate(this)
					});			
			var menuList;
    ...
    MoveStart:function(map){// 移动事件
    	map.addEventListener("movestart",function() {
			if (Ext.getCmp('metermap').DrawOperation==-1)	
    			this.initmap();
		}.createDelegate(this));
    },
