...
Ext.ux.GMapPanel = Ext.extend(Ext.Panel, {
    ...
    { 
    	if(!this.terminalDlg)
    	{
    		this.terminalDlg = new gMapTerminalCreate;
    	}
    	...
    	if (null != index)
    	{
    		   ...
    		   this.terminalDlg.setTitle('集中器信息[' + this.markInfors[index].name + ']') ;
    ...
    MoveStart:function(map){// 移动事件
    	map.addEventListener("movestart",function() {
			if (Ext.getCmp('terminalmap').DrawOperation==-1)	
    			this.initmap();
		}.createDelegate(this));
    },
    MoveEnd:function(map){ 
    	map.addEventListener("moveend",function() {
    		// 需要对标注进行重新绘制
    		var Pane = Ext.getCmp('terminalmap');
