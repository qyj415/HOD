...
<script>  
	...
	function changeUtils()
	{
		...
		if(utils == "1")
		{
			setSpan('the1','the2','the3');
			$("#days").val(1);
			$("#startTime").val('<%=Utils.getTime2(0,0)%>');
		}
		if(utils == "2")
		{
			setSpan('the2','the1','the3');
			$("#months").val(1);
			$("#startTime").val('<%=Utils.getTime(1,0)%>');
		}
		if(utils == "3")
		{
			setSpan('the3','the1','the2');
			$("#years").val(1);
			$("#startTime").val('<%=Utils.getTime3(1,0)%>');
		}
		$("#endTime").val('<%=Utils.dateToStr(new Date())%>');
	}
	
	function setSpan(id1,id2,id3)
	{
	...
	function selects()
	{
		...
		if(months==1)
			$("#startTime").val('<%=Utils.getTime(1,0)%>');
		if(months==2)
			$("#startTime").val('<%=Utils.getTime(3,0)%>');
		if(months==3)
			$("#startTime").val('<%=Utils.getTime(6,0)%>');
		if(months==4)
			$("#startTime").val('<%=Utils.getTime(12,0)%>');
		$("#endTime").val('<%=Utils.dateToStr(new Date())%>');
	}
	...
	function selects2()
	{
		...
		if(months==1)
			$("#startTime").val('<%=Utils.getTime2(0,0)%>');
		if(months==2)
			$("#startTime").val('<%=Utils.getTime2(7,0)%>');
		if(months==3)
			$("#startTime").val('<%=Utils.getTime2(15,0)%>');
		if(months==4)
			$("#startTime").val('<%=Utils.getTime(1,0)%>');
		$("#endTime").val('<%=Utils.dateToStr(new Date())%>');
	}
	...
	function selects3()
	{
		...
		if(months==1)
			$("#startTime").val('<%=Utils.getTime3(1,0)%>');
		if(months==2)
			$("#startTime").val('<%=Utils.getTime3(2,0)%>');
		if(months==3)
			$("#startTime").val('<%=Utils.getTime3(3,0)%>');
		if(months==4)
			$("#startTime").val('<%=Utils.getTime3(5,0)%>');
		$("#endTime").val('<%=Utils.dateToStr(new Date())%>');
	}
		...
		<div>
				<table class="repCnd">
				<tr class="repCnd">
				<td class="repCndLb" nowrap="nowrap">单位:</td>
				...
				<td class="repCndLb" nowrap="nowrap">常用日期选择:</td>
				...
				<td class="repCndEdit" nowrap="nowrap"><span>
					<input class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'endTime\',{d:-0});}'})" id="startTime"/>
				</span></td>
				<td class="repCndLb" nowrap="nowrap">结束时间:</td>
				<td class="repCndEdit" nowrap="nowrap">
					<span><input class="Wdate" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startTime\',{d:0});}'})" id="endTime"/> </span>
				</td>
				<td class="repCndEditRight"><input type="button" class="btn" value="" onclick="checkTime()" /></td>
				</tr>
				</table>
		</div>
