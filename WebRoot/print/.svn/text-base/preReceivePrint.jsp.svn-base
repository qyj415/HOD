							...
							<td>
								<table width="100%" border="0" cellpadding="0" cellspacing="1"
									 ...
									 <tr>
											...
											<td class="STYLE3">预收款金额(元)</td>
											<td class="STYLE3">收费时间</td>
											<td class="STYLE3">操作员</td>
											<td class="STYLE3">是否后付费</td>
										</tr>
										<c:forEach items="${dataList}" var="v" varStatus="num">
											<tr>
												...
												<td class="STYLE1"><f:formatDate value="${v.prTime }" pattern="yyyy-MM-dd"/></td>
												<td class="STYLE1">${v.prOperator }</td>
												<td class="STYLE1">
													<c:if test="${v.prAfter==1}">是</c:if>
													<c:if test="${v.prAfter==0}">否</c:if>
												</td>
											</tr>
										</c:forEach>
									<!--------------------------------------------------->
